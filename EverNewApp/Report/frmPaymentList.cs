...
namespace EverNewApp
{
    public partial class frmPaymentList : Form
    {
        ...
        void PopualteData()
        {
            string TM02_PARTYID = "", sT009_TYPE = "";
            int iTM02_PARTYID = 0;
            ...
            if (!string.IsNullOrEmpty(cmbType.Text.Trim()))
                sT009_TYPE = cmbType.SelectedItem.ToString();
            ...
            dt = dl.SelectMethod("exec USP_VP_GET_PAYMENT '" + TM02_PARTYID + "','" + sT009_TYPE + "','','','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptPaymentListing.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Payment Listing Report";
