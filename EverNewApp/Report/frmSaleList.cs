...
namespace EverNewApp
{
    public partial class frmSaleList : Form
    {
        ...
        void PopualteData()
        {
            ...
            //dt = dl.SelectMethod("exec USP_VP_GETSALELIST '','" + TM02_PARTYID + "','','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
           // dt = dl.SelectMethod("exec USP_VP_PRINT_SALE_BILL '' ");
            dt = dl.SelectMethod("exec USP_RPT_GETSALELIST '','" + TM02_PARTYID + "','','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptSaleListing.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Sale Listing Report";
