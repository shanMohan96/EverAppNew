...
namespace EverNewApp
{
    public partial class frmPurchaseList : Form
    {
        ...
        void PopualteData()
        {
            ...
            dt = dl.SelectMethod("exec USP_RPT_GET_PURCHASELIST '','" + TM02_PARTYID + "','','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptPurchaseListing.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Purchase Listing Report";
