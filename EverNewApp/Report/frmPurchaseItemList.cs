...
namespace EverNewApp
{
    public partial class frmPurchaseItemList : Form
    {
        ...
        void PopualteData()
        {
            ...
            dt = dl.SelectMethod("exec USP_RPT_GET_PURCHASE_ITEMS '" + TM02_PARTYID + "', '" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptPurchaseItemList.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Purchase Item Report";
