...
namespace EverNewApp
{
    public partial class frmOrderList : Form
    {
        ...
        void PopualteData()
        {
            ...
            dt = dl.SelectMethod("exec USP_VP_GET_ORDER '','" + TM02_PARTYID + "','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptOrderListing.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Order Listing Report";
