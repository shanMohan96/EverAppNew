...
{
    public partial class frmSaleItemList : Form
    {
        MyDabaseDataContext MyDa;
        DatabaseOperation dbo = new DatabaseOperation();

        public frmSaleItemList()
        {
            InitializeComponent();
        }
        ...
        void PopualteData()
        {
            ...
            dt = dl.SelectMethod("exec USP_RPT_GET_SALE_ITEM_REPORT '" + TM02_PARTYID + "'   ,'" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ReportDocument RptDoc = new ReportDocument();

                RptDoc.Load(Application.StartupPath + @"\Report\rptSaleItemReport.rpt");
                RptDoc.SetDataSource(dt);

                Datalayer.RptReport = RptDoc;
                Datalayer.sReportName = "Sale Item Report";
