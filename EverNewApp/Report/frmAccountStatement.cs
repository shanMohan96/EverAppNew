...
namespace EverNewApp
{
    public partial class frmAccountStatement : Form
    {
        ...
        void PopualteData()
        {
            ...
            dt = dl.SelectMethod("exec USP_VP_GET_STATEMENT '" + TM02_PARTYID + "','" + dtpFromDate.Value.ToString("yyyy-MM-dd") + "','" + dtpTodate.Value.ToString("yyyy-MM-dd") + "','" + Datalayer.iT001_COMPANYID + "' ");
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < tblOrdered.Rows.Count; i++)
                {
                    ...
                    if (i == 0)
                        tblOrdered.Rows[i]["Recive_Amount"] = 0;
                    else
                        tblOrdered.Rows[i]["Recive_Amount"] = System.Math.Abs(Recive_Amount);


                  //  tblOrdered.Rows[i]["Sale_Amount"] = dSale_Amount;
                ...
                RptDoc.Load(Application.StartupPath + @"\Report\rptStatement.rpt");
