...
{
    public partial class frmManageLoanPayment : Form
    {
        MyDabaseDataContext MyDa;
        DatabaseOperation dbo = new DatabaseOperation();

        public frmManageLoanPayment()
        {
            InitializeComponent();
        }
        ...
        {
            frmAddUpdateLoanPayment fmAdd = new frmAddUpdateLoanPayment();
        ...
        void PopualteData()
        {
            ...
            List<USP_VP_GET_WORKAR_PAYMENTResult> lst = new List<USP_VP_GET_WORKAR_PAYMENTResult>();
            lst = MyDa.USP_VP_GET_WORKAR_PAYMENT(TM02_PARTYID, dtpFromDate.Value, dtpTodate.Value, Datalayer.iT001_COMPANYID.ToString()).ToList();
            dgDisplayData.DataSource = lst;

            dgDisplayData.Columns["T017_WORKAR_PAYMENTID"].Visible = false;
            dgDisplayData.Columns["T14_WORKERID"].Visible = false;
            dgDisplayData.Columns["T017_DETAILS"].Visible = false;
            
            dgDisplayData.Columns["T14_NAME"].HeaderText = "Name";
            dgDisplayData.Columns["T017_DATE"].HeaderText = "Date";
            dgDisplayData.Columns["T017_DATE"].DefaultCellStyle.Format = "dd-MM-yyyy";
            dgDisplayData.Columns["T017_AMOUNT"].HeaderText = "Amount";

            dgDisplayData.Columns["T14_NAME"].DisplayIndex = 0;
            dgDisplayData.Columns["T017_DATE"].DisplayIndex = 1;
            dgDisplayData.Columns["T017_AMOUNT"].DisplayIndex = 2;
        ...
        void EditData()
        {
            ...
            {
                int ID = 0;
                int.TryParse(dgDisplayData.CurrentRow.Cells["T017_WORKAR_PAYMENTID"].Value.ToString(), out ID);

                Datalayer.iT017_WORKAR_PAYMENTID  = ID;

                frmAddUpdateLoanPayment frmAddSTD = new frmAddUpdateLoanPayment();
            ...
            if (dgDisplayData.SelectedRows.Count > 0)
            {
                ...
                if (Datalayer.ShowQuestMsg(Datalayer.sMessageConfirmation))
                {
                    try
                    {
                        ...
                        int? Iout = 0;
                        MyDa = new MyDabaseDataContext(Properties.Settings.Default.Style_King_Dev);
                        string Sq = "DELETE FROM T017_WORKAR_PAYMENT WHERE T017_WORKAR_PAYMENTID=" + ID;
                        DAL dl = new DAL();
                        if (dl.ExecuteMethod(Sq))
                        {
                            ...
                            Datalayer.DosenotDeleteMessageBox("Loan Payment Details");
                    }
                    catch (Exception)
                    {
