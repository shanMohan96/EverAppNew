...
{
    public partial class frmAddUpdateLoanPayment : Form
    {
        ...
        public frmAddUpdateLoanPayment()
        {
            InitializeComponent();
        }
        ...
        void PopauteData()
        {
            if (Datalayer.iT15_LOANID > 0)
            {
                ...
                dt = dl.SelectMethod("SELECT * FROM T017_WORKAR_PAYMENT WHERE T017_WORKAR_PAYMENTID=" + Datalayer.iT017_WORKAR_PAYMENTID);
                if (dt.Rows.Count > 0)
                {
                    cmbEmployee.SelectedValue = dt.Rows[0]["T14_WORKERID"].ToString();
                    txtAmount.Text = dt.Rows[0]["T017_AMOUNT"].ToString();
                    dtpDate.Text = dt.Rows[0]["T017_DATE"].ToString();
                    txtDetails.Text = dt.Rows[0]["T017_DETAILS"].ToString();
        ...
        void AddUpateBank()
        {
            try
            {
                ...
                int? Iout = 0;
                MyDa.USP_VP_ADDUPDATE_WORKAR_PAYMENT(Datalayer.iT017_WORKAR_PAYMENTID, T14_WORKERID, dtpDate.Value, dT14_AMOUNT, txtDetails.Text.Trim(), Datalayer.iT001_COMPANYID, ref Iout);
                if (Iout > 0)
                {
                    if (Datalayer.iT017_WORKAR_PAYMENTID == 0)
        ...
        void ResetData()
        {
            Datalayer.iT017_WORKAR_PAYMENTID = 0;
            ep1.Clear();
        ...
        {
            frmManageLoanPayment fm = new frmManageLoanPayment();
